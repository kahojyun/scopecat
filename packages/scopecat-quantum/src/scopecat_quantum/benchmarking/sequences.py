...
SINGLE_QUBIT_INTERLEAVED_RB_PROTOCOL = "scopecat.rb.1q.clifford.interleaved.v1"
...
@dataclass(frozen=True, slots=True)
class SingleQubitInterleavedRbSequence:
    """One reference 1q Clifford backbone with a chosen realization interleaved."""

    reference_key: SequenceKey
    length: int
    random_cliffords: tuple[int, ...]
    interleaved_primitives: tuple[SingleQubitPrimitive, ...]
    interleaved_clifford: int
    recovery_clifford: int
    ...
    @property
    def cliffords(self) -> tuple[int, ...]:
        """Return the executed Clifford order, including the exact recovery."""

        return (
            *(
                clifford
                for random_clifford in self.random_cliffords
                for clifford in (random_clifford, self.interleaved_clifford)
            ),
            self.recovery_clifford,
        )
    ...
    def fingerprint(self) -> str:
        return _fingerprint(
            {
                "cliffords": self.cliffords,
                "derived_seed": self.reference_key.derived_seed,
                "interleaved_primitives": self.interleaved_primitives,
                "length": self.length,
                "primitives": self.primitives,
                "protocol": SINGLE_QUBIT_INTERLEAVED_RB_PROTOCOL,
...
def _single_qubit_primitive_clifford(gate: SingleQubitPrimitive) -> int:
    for clifford in range(single_qubit_clifford_count()):
        if decompose_single_qubit_clifford(clifford) == (gate,):
            return clifford
    raise ValueError(f"unsupported single-qubit interleaved gate: {gate!r}")


def single_qubit_interleaved_rb_sequence(
    root_seed: int,
    length: int,
    *,
    interleaved_primitives: tuple[SingleQubitPrimitive, ...],
    ...
    """Interleave one realized Clifford with a reference RB backbone.

    Random Cliffords are generated by :func:`single_qubit_rb_sequence`, so the
    same seed, length, sample index, member, and length-sampling mode select the
    exact reference backbone.  The recovery is recomputed for the alternating
    random-Clifford/interleaved-gate circuit.
    """

    reference = single_qubit_rb_sequence(
        root_seed,
        length,
        sample_index=sample_index,
        member_id=member_id,
    ...
    if not interleaved_primitives:
        raise ValueError("single-qubit interleaved realization must not be empty")
    interleaved_clifford = single_qubit_clifford_product(
        tuple(
            _single_qubit_primitive_clifford(primitive)
            for primitive in interleaved_primitives
        )
    )
    executed_cliffords = tuple(
        clifford
        for random_clifford in reference.random_cliffords
        for clifford in (random_clifford, interleaved_clifford)
    )
    recovery = single_qubit_clifford_inverse(
        single_qubit_clifford_product(executed_cliffords)
    )
    primitives = cast(
        "tuple[SingleQubitPrimitive, ...]",
        tuple(
            ...
            for random_clifford in reference.random_cliffords
            for primitive in (
                *decompose_single_qubit_clifford(random_clifford),
                *interleaved_primitives,
            )
        )
        + decompose_single_qubit_clifford(recovery),
    )
    return SingleQubitInterleavedRbSequence(
        reference_key=reference.key,
        length=length,
        random_cliffords=reference.random_cliffords,
        interleaved_primitives=interleaved_primitives,
