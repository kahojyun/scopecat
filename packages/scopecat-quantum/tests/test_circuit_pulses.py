...
def test_zero_event_gate_still_exports_an_instantiation_and_result_is_sealed() -> None:
    call = _call("empty")
    ...
    with pytest.raises(TypeError, match="only be created"):
        LoweredCircuitPulseProgram(
            lowered.source_circuit_id,
            lowered.program,
            lowered.instantiations,
            lowered.event_provenance,
            lowered.acquisition_provenance,
        )
    attribute = "program"
...
def test_lowering_defensively_rejects_unknown_template_nodes() -> None:
    call = _call("call")
    verified = _verified(call)
    calibration = _calibration("calibration", call, _template(Q0, 1))
    selection = select_calibrations(
        verified,
        CalibrationCatalog(gates=GateCalibrationCatalog((calibration,))),
    )
    malformed_template = PulseProgram(
        id=PulseProgramId("malformed"),
        body=cast(
            "PulseInstruction",
            _AlienPulseNode((calibration.pulse_template.body,)),
        ),
    )
    object.__setattr__(
        selection.gates.bindings[0],
        "pulse_template",
        malformed_template,
    )
    ...
    assert tuple(issue.code for issue in raised.value.issues) == (
        CircuitPulseLoweringIssueCode.TEMPLATE_STRUCTURE_INVALID,
    )


def test_lowering_aggregates_every_independent_template_refinement_failure() -> None:
    calls = (
        _call("call-0", Q0),
        _call("call-1", Q1),
        _call("call-2", Q2),
    ...
    wrong_program_id = PulseProgram(
        id=cast("PulseProgramId", CircuitId("wrong-space")),
        body=calibrations[0].pulse_template.body,
    )
    acquire_signal = AcquireSignal(Q1)
    acquisition_slot = AcquisitionSlot(
        AcquisitionSlotId("slot"),
        AcquisitionKind.INTEGRATED_IQ,
        acquire_signal,
    )
    acquisition_template = PulseProgram(
        id=PulseProgramId("acquisition-template"),
        body=Acquire(
        ...
        acquisition_slots=(acquisition_slot,),
    )
    duplicate_event_id = PulseEventId("duplicate")
    duplicate_template = PulseProgram(
        id=PulseProgramId("duplicate-template"),
        body=PulseSequence(
            (
                Delay(duplicate_event_id, DriveSignal(Q2), Quantity(1, "ns")),
                Delay(duplicate_event_id, DriveSignal(Q2), Quantity(1, "ns")),
            )
        ),
    )
    invalid_event_template = PulseProgram(
        id=PulseProgramId("invalid-event-template"),
        body=Delay(
            cast("PulseEventId", CircuitOperationId("wrong-space")),
            DriveSignal(Q3),
            Quantity(1, "ns"),
        ),
    )
    malformed_templates = (
        ...
        duplicate_template,
        invalid_event_template,
    )
    for binding, template in zip(
        selection.gates.bindings,
        malformed_templates,
        strict=True,
    ):
        object.__setattr__(binding, "pulse_template", template)

    with pytest.raises(CircuitPulseLoweringError) as raised:
        lower_circuit_to_pulses(
            verified,
            selection,
            output_id=PulseProgramId("invalid-templates"),
        )
    ...
    assert tuple(
        ...
        (
            CircuitPulseLoweringIssueCode.TEMPLATE_PROGRAM_ID_INVALID,
            (
                "selection",
                "gates",
                "bindings",
        ...
        (
            CircuitPulseLoweringIssueCode.TEMPLATE_ACQUISITION_UNSUPPORTED,
            ("selection", "gates", "bindings", 1, "pulse_template"),
            calls[1].id,
            calibrations[1].id,
            None,
        ),
        (
            CircuitPulseLoweringIssueCode.TEMPLATE_EVENT_DUPLICATE,
            (
                "selection",
                "gates",
                "bindings",
        ...
        (
            CircuitPulseLoweringIssueCode.TEMPLATE_EVENT_ID_INVALID,
            (
                "selection",
                "gates",
                "bindings",
