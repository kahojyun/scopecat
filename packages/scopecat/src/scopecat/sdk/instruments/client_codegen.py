"""Generate committed typed instrument surfaces from Python declarations."""
...
def package_generation_targets(
    manifest: InstrumentClientManifest,
    /,
    *,
    package_module: str,
    output_root: Path,
...
def _load_manifest(symbol: str, /) -> InstrumentClientManifest:
    ...
    if not all(
        hasattr(manifest, field)
        for field in ("surfaces", "public_types", "static_exports")
    ):
        raise TypeError(f"{symbol!r} is not an instrument client manifest")
    return cast("InstrumentClientManifest", manifest)


type RenderedSource = tuple[Path, str]


def render_package_sources(
    targets: PackageGenerationTargets,
    /,
    *,
    additional_catalog_targets: tuple[CatalogTarget, ...] = (),
) -> tuple[RenderedSource, ...]:
    """Render one package and any caller-owned auxiliary catalogs in one pass."""
    ...
    return (
        ...
        *(
            rendered_source
            for target in (targets.catalog, *additional_catalog_targets)
...
def write_package_sources(
    rendered: tuple[RenderedSource, ...],
    /,
    *,
    check: bool = False,
) -> tuple[Path, ...]:
    """Write rendered sources, or return stale paths without mutating in check mode."""

    stale = tuple(
        output
        for output, source in rendered
        if (output.read_text(encoding="utf-8") if output.is_file() else "") != source
    )
    if check:
        return stale
    for output, source in rendered:
...
def generate_instrument_package(
    manifest: InstrumentClientManifest,
    /,
    *,
    package_module: str,
    ...
    """Generate or check all committed modules for one instrument package."""

    targets = package_generation_targets(
        manifest,
        package_module=package_module,
        output_root=output_root,
    )
    return write_package_sources(
        render_package_sources(
            targets,
            additional_catalog_targets=additional_catalog_targets,
        ),
        check=check,
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if stale:
        ...
        print(
            ...
            "scopecat-generate-instrument-clients without `--check`",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
__all__ = [
    ...
    "main",
    "package_generation_targets",
    "render_catalog_target",
