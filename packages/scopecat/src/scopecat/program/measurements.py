"""Point-local measurement compute contracts."""
...
class MeasurementPostprocessor:
    ...
    def __post_init__(self) -> None:
        if not self.id:
            raise ValueError("measurement compute ids must be non-empty")
        if not self.input_bindings:
            raise ValueError("measurement computes require a measured input")
        if any(not name for name, _product in self.input_bindings):
            raise ValueError("measurement compute input names must be non-empty")
        if any(not name for name, _value in self.value_input_bindings):
            raise ValueError("measurement compute value names must be non-empty")
        if any(not role for role, _product in self.output_bindings):
            raise ValueError("measurement compute output roles must be non-empty")
        if not self.output_bindings:
            raise ValueError("measurement computes require at least one output")
        if any(not segment for segment in self.scope):
            raise ValueError("measurement compute scope must contain non-empty strings")
        ...
        if input_ids & output_ids:
            raise ValueError("measurement compute inputs and outputs must be distinct")
        _require_unique(
