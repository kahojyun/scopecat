...
from scopecat.domain_invocation import ClosedDomainInvocation, DomainInvocationIntent
...
_KNOWN_SUBMISSION_TOKEN = object()
_ABSENT_SUBMISSION_TOKEN = object()
_UNCERTAIN_SUBMISSION_TOKEN = object()
_CORRELATED_FETCH_TOKEN = object()
_PENDING_FETCH_TOKEN = object()
...
class KnownDomainSubmission:
    ...
    def __init__(
        ...
        *,
        ...
        if _token is not _KNOWN_SUBMISSION_TOKEN:
            msg = "KnownDomainSubmission can only be created by core runtime checks"
            raise TypeError(msg)
        if origin == "submit":
...
class AbsentDomainSubmission:
    ...
    def __init__(
        ...
        *,
        ...
        if _token is not _ABSENT_SUBMISSION_TOKEN:
            msg = "AbsentDomainSubmission can only be created by core runtime checks"
            raise TypeError(msg)
        valid = (
            origin == "submit"
...
class UncertainDomainSubmission:
    ...
    def __init__(
        ...
        *,
        ...
        if _token is not _UNCERTAIN_SUBMISSION_TOKEN:
            msg = "UncertainDomainSubmission can only be created by submit handling"
            raise TypeError(msg)
...
class CorrelatedDomainFetch[ResultT]:
    ...
    def __init__(
        ...
        *,
        ...
        if _token is not _CORRELATED_FETCH_TOKEN:
            msg = "CorrelatedDomainFetch can only be created by fetch correlation"
            raise TypeError(msg)
...
class PendingDomainFetch:
    ...
    def __init__(
        ...
        *,
        ...
        if _token is not _PENDING_FETCH_TOKEN:
            msg = "PendingDomainFetch can only be created by fetch correlation"
            raise TypeError(msg)
    ...
    *,
    ...
    if receipt.status == "submitted":
        known = KnownDomainSubmission(
            ...
            receipt,
            "submit",
            _token=_KNOWN_SUBMISSION_TOKEN,
        )
        _append_after_effect(
            journal,
            started.model_copy(
    ...
    if receipt.status == "not_submitted":
        absence = AbsentDomainSubmission(
            ...
            receipt,
            "submit",
            _token=_ABSENT_SUBMISSION_TOKEN,
        )
        _append_after_effect(
            journal,
            started.model_copy(
    ...
    *,
    ...
    if receipt.status == "fetched":
        ...
        return CorrelatedDomainFetch(
            ...
            receipt,
            candidate.result,
            _token=_CORRELATED_FETCH_TOKEN,
        )
    if receipt.status == "pending":
        return PendingDomainFetch(
            submission,
            receipt,
            _token=_PENDING_FETCH_TOKEN,
        )
    raise DomainFetchFailed(
        receipt_problems,
        run_id=attempt.run_id,
...
def _require_intent(value: object) -> None:
    if not isinstance(value, DomainInvocationIntent):
        ...
        raise TypeError(msg)
    DomainInvocationIntent.model_validate(value.model_dump(mode="json"))
...
def _validate_submission_id(
    ...
    if not isinstance(cast("object", attempt), DomainSubmissionId):
        ...
        raise TypeError(msg)
    normalized = DomainSubmissionId.model_validate(attempt.model_dump(mode="json"))
    if (
        normalized.invocation_id != intent.invocation_id
        or normalized.intent_fingerprint != intent.intent_fingerprint
    ):
...
def _require_known_submission(
    ...
    if not isinstance(cast("object", submission), KnownDomainSubmission):
        ...
        raise TypeError(msg)
    normalized_receipt: DomainSubmitReceipt | DomainReconcileReceipt
    if isinstance(submission.receipt, DomainSubmitReceipt):
        normalized_receipt = _normalize_submit_receipt(submission.receipt)
    else:
        normalized_receipt = _normalize_reconcile_receipt(submission.receipt)
    normalized = KnownDomainSubmission(
    ...
    if normalized != submission:
        msg = "known domain submission no longer matches its sealed state"
        raise ValueError(msg)
...
def _require_absence(
    ...
    normalized_receipt: DomainSubmitReceipt | DomainReconcileReceipt
    if isinstance(sealed_absence.receipt, DomainSubmitReceipt):
        normalized_receipt = _normalize_submit_receipt(sealed_absence.receipt)
    else:
        normalized_receipt = _normalize_reconcile_receipt(sealed_absence.receipt)
    normalized = AbsentDomainSubmission(
        sealed_absence.submission_id,
        normalized_receipt,
        sealed_absence.origin,
        _token=_ABSENT_SUBMISSION_TOKEN,
    )
    if normalized != sealed_absence:
        msg = "absent domain submission no longer matches its sealed state"
        raise ValueError(msg)
...
def _require_uncertainty(
    ...
    normalized = UncertainDomainSubmission(
        sealed_uncertainty.submission_id,
        DomainReceiptIdentity.model_validate(
            sealed_uncertainty.identity.model_dump(mode="json")
        ),
        reason=sealed_uncertainty.reason,
        submit_call_attempt=sealed_uncertainty.submit_call_attempt,
        job_id_hint=sealed_uncertainty.job_id_hint,
        problems=tuple(
            Problem.model_validate(problem.model_dump(mode="json"))
            for problem in sealed_uncertainty.problems
        ),
        _token=_UNCERTAIN_SUBMISSION_TOKEN,
    )
    if normalized != sealed_uncertainty:
        msg = "uncertain domain submission no longer matches its sealed state"
        raise ValueError(msg)
