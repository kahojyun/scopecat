...
from scopecat._compiler.program import TypedProgram, product_output, record_product
...
from scopecat.execution_journal import (
    ExecutionTransition,
...
def test_runtime_state_capabilities_cannot_be_constructed_by_adapters() -> None:
    invocation = _closed_invocation()
    submission_id = _submission_id(invocation)
    identity = domain_receipt_identity(submission_id, invocation.intent)
    ...
    with pytest.raises(TypeError, match="KnownDomainSubmission"):
        KnownDomainSubmission(submission_id, submitted, "submit")
    with pytest.raises(TypeError, match="AbsentDomainSubmission"):
        AbsentDomainSubmission(submission_id, not_submitted, "submit")
    ...
    with pytest.raises(TypeError, match="CorrelatedDomainFetch"):
        CorrelatedDomainFetch(known, fetched, "payload")
    with pytest.raises(TypeError, match="PendingDomainFetch"):
        PendingDomainFetch(known, pending)


def test_fetch_and_reconcile_reject_forged_state_before_runtime_calls() -> None:
    invocation = _closed_invocation()
    submission_id = _submission_id(invocation)
    runtime = _ScriptedRuntime()
