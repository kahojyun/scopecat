...
from scopecat._relations import col, grid, literal_rows, outer, point_col
...
def test_binding_cannot_bypass_program_proof_role_verification() -> None:
    environment = validate_config_environment(load_config())

    plan = bind_program(_fictional_current_row_state(), environment)

    assert not plan.valid
    assert plan.problems[0].code == "compiler_relation_proof_role_mismatch"
    assert plan.problems[0].phase.value == "planning"
    assert plan.points == ()
