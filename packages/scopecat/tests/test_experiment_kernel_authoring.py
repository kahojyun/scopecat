...
from tests.support.experiment_kernel import parameter_build
...
def test_rows_helper_selects_parameter_table_rows() -> None:
    ...
    assert [point.row["device_id"] for point in plan.points] == ["r0"]
    assert plan.point_coordinate_ids == [
        "device_id",
        "enabled",
        "resource_id",
        "frequency",
...
def test_scan_parameter_helper_builds_points_and_patch() -> None:
    ...
    spec = experiment(
        ...
        acquire=acquire("iq", observations=[point("signal", unit="ratio")]),
    )

    plan = plan_experiment(spec, parameter_build())

    assert plan.point_coordinate_ids == ["readout_frequency"]
    assert [point.row["readout_frequency"] for point in plan.points] == [
        Quantity(value=5.9, unit="GHz"),
        Quantity(value=6.0, unit="GHz"),
    ]
    assert [
        (record.patch.key or {})["device_id"] for record in plan.parameter_patches
    ] == ["r0", "r0"]
    assert [
...
def test_local_scan_helper_records_center_offset_and_actual_value() -> None:
    ...
    assert [point.row["frequency_center"] for point in plan.points] == [
        Quantity(value=5.95, unit="GHz")
    ] * 3
    assert [point.row["frequency_offset"] for point in plan.points] == [
        Quantity(value=-50, unit="MHz"),
        Quantity(value=0, unit="MHz"),
        Quantity(value=50, unit="MHz"),
    ]
    assert [point.row["frequency"] for point in plan.points] == [
        Quantity(value=5.9, unit="GHz"),
        Quantity(value=5.95, unit="GHz"),
        Quantity(value=6.0, unit="GHz"),
    ]
...
def test_local_overrides_helper_builds_axes_and_state() -> None:
    ...
    assert [
        ...
        if record.resource == "C12" and record.field == "z_pulse.amplitude"
    ] == [Quantity(value=0.5, unit="V")] * 6
