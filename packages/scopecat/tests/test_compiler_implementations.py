...
from scopecat._relations import RelationExpr, Row, literal_rows
...
from scopecat.value_types import Float, Payload, Scalar, String, Table, ValueType
from tests.support.authoring import load_config
from tests.support.relation_plans import point_domain
...
def test_binding_selects_stable_implementation_and_hashes_its_identity() -> None:
    ...
    first_plan = bind_program(first, environment)
    second_plan = bind_program(second, environment)
...
def test_implementation_id_versions_cache_while_plan_pins_exact_callable() -> None:
    ...
    environment = validate_config_environment(load_config())
    plan = bind_program(first_program, environment)
    second_plan = bind_program(second_program, environment)
...
def test_compute_result_identity_does_not_change_value_cache_semantics() -> None:
    ...
    first = bind_program(
        _program(catalog=catalog, output_id=first_output),
        environment,
    )
    second = bind_program(
        _program(catalog=catalog, output_id=second_output),
        environment,
    )
...
def test_compute_cache_identity_includes_selected_typed_interface() -> None:
    ...
    float_plan = bind_program(_program(catalog=catalog), environment)
    string_plan = bind_program(
        _program(catalog=catalog, output_type=Scalar(String())),
        environment,
    )
