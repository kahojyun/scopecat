...
from scopecat._relations import RelationExpr, lit, literal_rows, point_col
...
def test_bind_program_returns_diagnostic_plan_for_duplicate_compute_operations() -> (
    ...
    assert plan.points == ()
    assert len(plan.compute_definitions) == 2
    assert [problem.code for problem in plan.problems] == [
        "compute_operation_duplicate"
    ]


def test_bind_program_returns_diagnostic_plan_for_duplicate_compute_outputs() -> None:
    ...
    assert plan.points == ()
    assert len(plan.compute_definitions) == 2
    assert [problem.code for problem in plan.problems] == ["compute_output_duplicate"]
