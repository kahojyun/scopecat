...
from tests.testkit.run_overview import (
...
def test_build_run_overview_for_signal_run_does_not_update_manifest(
    ...
    assert local_run_repository(tmp_path).read_manifest(run_id).status == "completed"
...
def test_build_run_overview_for_full_local_workflow(
    ...
    assert sorted(
        [
            ...
            for artifact in local_run_repository(tmp_path)
...
def test_build_run_overview_includes_manual_analysis_records(
    ...
    overview = build_run_overview(
        run_id=run.id, services=local_workspace_services(tmp_path)
    )
