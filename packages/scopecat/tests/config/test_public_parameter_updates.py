...
def test_public_parameter_update_builders_return_canonical_models() -> None:
    ...
    assert updates[1].value.shape == "table"
    assert updates[2].key == {"channel": "q0"}
    assert updates[2].values == {"gain": 0.75}
    assert updates[3].rows == ({"channel": "q1", "gain": 0.25},)
    assert updates[4].key == {"channel": "q2"}
    assert isinstance(updates[2].key, FrozenMapping)
    assert isinstance(updates[2].values, FrozenMapping)
    assert all(isinstance(row, FrozenMapping) for row in updates[3].rows)
    assert isinstance(updates[4].key, FrozenMapping)
