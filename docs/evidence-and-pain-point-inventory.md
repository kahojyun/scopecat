...
| JC-002 | 2 | Starred runs to analysis handoff package | User refinement raises internal analysis handoff above cross-machine scientific comparison as a nearer adoption pressure. This tests PN-004 with EV-044 and EV-045: users need to find high-value runs, inspect simple historical previews, mark or multi-select them, and move an immutable data-plus-context snapshot from an experiment-control computer to a personal analysis computer without losing source identity. | Candidate details live in [`analysis-handoff-candidate-note.md`](analysis-handoff-candidate-note.md). Do not accept public/reference packaging, full publication workflow, generic export-format-first design, full work-bundle export/import, live-monitor semantics, managed analysis-script execution, permission systems, or reader/API/storage/UI/schema details. |
