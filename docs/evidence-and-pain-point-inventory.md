...
- Data acquisition, quick inspection, deeper analysis, reporting, and method
  ...
  conventions.
- A high-priority handoff case is not cross-machine scientific comparison, but
  moving high-value data and context off an experiment-control computer for
  deeper analysis, publication figures, and reporting so the control computer
  can return to experiment work. In permissive internal lab settings, this
  usually needs full-fidelity context rather than redaction.
- Users often find high-value data through experiment names, measurement-object
  labels, quick plots, and simple favorite/star marks. If Scopecat cannot help
  users find, mark, and take away valuable data, adoption pressure increases
  even if the underlying records are more complete than legacy systems.
- Analysis handoff should prioritize Scopecat's high-fidelity data model and a
  usable read API over forcing early export into a generic interchange format.
  Mainstream analysis ecosystem adapters or guides can follow observed use.
- Predecessor Fricon data-shape work is enough to preserve an early baseline:
  regular one-, two-, and N-dimensional sweeps; trace-valued records with
  explicit coordinate/value arrays or compact regular-coordinate descriptions;
  multiple traces in one outer sweep record; first-class complex values for
  magnitude/phase or I/Q views; IQ averages, I/Q channels, single-shot arrays,
  or labels; and generic irregular or ragged step records for minimizer-like
  output. W2 should treat this as adoption and validation pressure, not as an
  accepted storage model, reader API, export format, or UI specification.
- Moving code or methods between experiment-control computers is a different
  ...
  code folders, but useful Scopecat support should treat copied material as
  reference, template, seed values, or readiness evidence rather than importing
  a complete work bundle as active setup, parameter, or execution truth.
...
| PN-004 | Strong follow-on W2 candidate. | direct; inferred | Handoff fixture linking selected or starred analysis-ready data, figures, fit results, reports, spreadsheets, and decks back to source runs, context, code, corrections, and decisions. | I cannot find and take high-value data off the experiment computer for deeper analysis, figure making, or publication without losing links back to source runs, companion artifacts, corrections, and decisions. | EV-002, EV-008, EV-009, EV-013, EV-017, EV-022, EV-028 | Good follow-on if W2 first establishes bundle identity across context, code, and generated artifacts. Prioritize internal full-fidelity analysis handoff before public/reference export. A minimal useful shape is a post-run browser with free names, stable IDs/read API, star/favorite marks, quick previews, multi-select self-contained immutable handoff packages, automatic context inclusion with evidence-handling labels, explicit missing-context warnings, code references plus key script snapshots, and optional fit-result/figure/star write-back. Use predecessor Fricon's data-shape list as validation coverage, but defer first-contact reader view, API signatures, package manifest fields, storage model, and format adapters until this journey is selected for specification. Avoid a full ELN, report generator, generic export-format-first design, permission system, or complete work-bundle export/import. |
...
| PN-029 | Capability-gap follow-on; analysis-impact led. | direct; inferred | Figure or claim impact fixture linking raw data, processing notebooks, generic fit-result tables, exclusions, calibrations, setup context, and rerun history. | I cannot tell which figures, fits, reports, or conclusions need rechecking after a calibration, setup, code, or analysis change. | EV-009, EV-017, EV-022, EV-038, EV-039 | Keep this as analysis lineage and impact review. Early fit records should be generic result tables with input run references, parameter names, values, units, errors, and quality metrics rather than experiment-specific schemas. Avoid turning W2 into a full publication workflow, ELN, or report generator. |
