...
| Handoff and publication | Good evidence for derived artifacts and source identity loss; user-context pressure makes analysis handoff from control computer to personal analysis computer a high-priority adoption case. | Reports, figures, spreadsheets, decks, analysis-ready data packages, run names, star/favorite marks, quick previews, fit-result tables, read API expectations, and the missing links back to run, context, code, correction, and decision records. | Whether analysis handoff should be a follow-on journey after bundle identity, or part of the first fixture; public/reference sharing may need curated analysis datasets plus provenance summaries rather than complete work-bundle export. |
