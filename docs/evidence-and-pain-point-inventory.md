...
| JC-016 | 16 | Bounded automation handoff to a lab-owned runtime | Tests the bridge from `JC-007`, `JC-008`, `JC-003`, and `JC-011` into real automation: a frozen plan or queue package with runtime owner, supplied bounds, readiness evidence, review gates, lifecycle, stop behavior, failure policy, and audit records can be handed to a lab-owned runtime for a narrow apply or unattended execution case. | First real-automation boundary only; no Scopecat-owned instrument runtime by default, generic scheduler, workflow DAG, environment sync, package deployment, broad resource-locking system, rollback guarantee, or autonomous calibration platform. |
