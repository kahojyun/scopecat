...
- Primary acceptance pressure for that candidate: `PN-002`, `PN-006`,
  `PN-005`, and `PN-001`.
- Primary constraints: adoption blockers `PN-020` through `PN-025`, plus
  portability and low-ceremony constraints from `PN-007` and `PN-016`.
...
foundational pains can drive acceptance, adoption blockers constrain the
journey, JTBD candidates need journey phrasing, capability gaps need fixtures,
and framework baselines are substrate or validation detail rather than primary
product pain.
...
why portability, low ceremony, cross-machine trust, and technical-owner
boundaries matter during W2.
...
- Researchers may need to run their own experiment plan on different equipment
  systems because of scheduling, availability, cooldown state, sample state, or
  lab coordination.
...
TP-006, TP-007, TP-008, PN-004, PN-005, PN-007, PN-016, PN-018, PN-020 through
PN-025, and PN-026 through PN-032. It should not promote Windows support,
deployment management, driver maintenance, lab IT administration, onboarding,
training, environment management, rollback automation, a universal experiment
schema, exhaustive lab inventory management, or a full ELN into early scope
unless a journey explicitly needs them.
...
| --- | --- | --- |
| [QCoDeS](https://microsoft.github.io/Qcodes/examples/basic_examples/15_minutes_to_QCoDeS.html) | Python instrument parameters, measurement loops, datasets, experiment/sample metadata, and station/instrument snapshots. | Do not compete by merely saving run metadata or instrument snapshots; focus on selected context, code provenance, cross-machine readiness, and legacy bundle explanation around existing scripts. |
...
| EV-026 | Multi-system, Windows-heavy lab context strengthens cross-machine trust and portability pressure. | Evidence-backed inference | Medium | user context; workflow improvement case; engineering guardrail | Generic lab context includes multiple non-identical equipment systems, one effective instrument-control computer per station, occasional movement across systems, and limited appetite for environment repair during experiments. | Cross-machine readiness; clean or share a run safely; validate a run package before execution. |
...
| EV-031 | Live control-PC fragility is an adoption blocker distinct from ordinary portability. | Evidence-backed inference | Medium | blind persona check; user context; engineering guardrail | Blind persona roles rejected heavy agents, fragile services, accounts, network dependencies, cloud login, or automatic driver, PATH, registry, vendor stack, and environment mutation on working Windows control computers. | Low-risk local adoption; managed execution readiness; control-PC read-only companion validation. |
| EV-032 | A known-good reference is a natural anchor for comparison, migration, rollback, and handoff. | Latent pressure | Medium | blind persona check; workflow improvement case; user context | Blind persona roles repeatedly framed the question as why last month's working run fails today, which setup state is proven to work, and how to recover after driver, package, or config changes. Workflow evidence shows folder copies, backups, run families, and local exports used as weak substitutes for known-good references. | Compare current state to a reference; cross-machine readiness; take over an old experiment. |
| EV-033 | Diagnostic snapshots need explicit sharing boundaries, not blanket redaction. | Evidence-backed inference | Medium | blind persona check; user context; engineering guardrail | Blind persona roles wanted precise, shareable support context instead of screenshots or vague failure reports. For the current internal-lab use case, local paths, hostnames, instrument addresses, and LabRAD or VISA details may be essential debugging evidence; redaction becomes valuable mainly for public docs, external exports, or other boundary crossings. | Ask a maintainer for help; clean or share a run safely; support handoff. |
...
| --- | --- | --- | --- |
| PN-001, PN-002, PN-005, PN-006 | Foundational pains | Natural decompositions of TP-002, TP-004, TP-005, and TP-006. | Keep as the primary JC-001 acceptance cluster. |
...
| PN-004, PN-010, PN-011, PN-012, PN-019, PN-029, PN-030 | Lineage, handoff, and campaign validation cases | Natural from TP-003, TP-005, TP-006, and TP-008 when the fixture contains generated protocols, correction branches, run families, figures, or manual interventions. | Pull into W2 only when the selected fixture needs them; otherwise keep as follow-on validation. |
| PN-020, PN-021, PN-022, PN-023, PN-024, PN-025 | Adoption blockers and guardrails | Natural decomposition of TP-007. | Constrain early journeys around truth drift, false confidence, control-PC safety, share-boundary handling, known-good references, and recovery evidence. |
| PN-026, PN-027, PN-028, PN-031, PN-032 | Capability-gap pains | Natural decomposition of TP-008. | Use to test differentiation around comparability, full-stack diff, setup reality, metadata ROI, and local schema evolution. |
| PN-007, PN-015, PN-016 | Adoption affordances and portability guardrails | Support TP-002, TP-005, TP-006, and TP-007, but are not the center of a pain narrative. | Keep as acceptance constraints around portability, low ceremony, and quick handoff affordances. |
...
```text
I cannot trust the experiment system configuration on another computer, so I
copy code and context before using it.
```

A corresponding JTBD should be more situational:

```text
When I need to move an experiment plan or inherited bundle to another setup,
help me compare it with a known-good reference across code, environment,
...
| PN-022 | Adoption constraint; control-PC safety led. | inferred; future/ADR | Low-risk read-only companion prototype or installation review on a representative Windows control-PC workflow. | I cannot adopt a tool that adds fragile services, accounts, network dependencies, cloud login, or automatic driver/environment mutation to a working control computer. | EV-026, EV-027, EV-031 | Make low-risk operation an acceptance constraint for early slices. Do not accept background agents, mandatory network services, package management, driver mutation, or registry/PATH changes from W1/W2. |
| PN-025 | Supporting rollback concern; maintainer led. | inferred; future/ADR | Change-impact or rollback-evidence fixture that records what changed and what known-good reference to return to, without performing rollback. | I cannot recover a working experiment state quickly after a dependency, driver, vendor tool, or config change breaks the setup. | EV-007, EV-026, EV-027, EV-031, EV-032 | Preserve rollback pressure as diagnostic and reference evidence first. Automatic restore, environment management, and driver rollback are later decisions. |
| PN-023 | Support and share-boundary constraint. | inferred | Boundary-aware diagnostic-package fixture for asking a trusted internal maintainer to debug a failed or questionable run, with sanitization only when the package crosses a public, external, or explicitly restricted boundary. | I cannot ask a maintainer for help cleanly because the useful diagnostic context is either missing and vague, or bundled with local details whose sensitivity depends on who receives it. | EV-013, EV-022, EV-027, EV-033 | Preserve full-fidelity internal diagnostics by default; redaction should be an export or support-boundary mode, not an internal-lab default. Do not make this a general bug-reporting product in W2. |
...
| JC-009 | 2 | Compare a known-good reference with a current bundle or machine | Blind persona checks repeatedly framed adoption around "last month worked, today fails" and "can I move this plan to another setup before wasting the slot." This tests PN-005, PN-020, PN-021, PN-022, PN-024, and PN-025 while keeping the first slice diagnostic. | Comparison, confidence gaps, and diagnostic evidence only; no rollback automation, package installation, driver mutation, cloud service, authoritative config write-back, or device control. |
| JC-010 | 3 | Compare scientific comparability between two runs, setups, or method variants | External framework baselines and blind capability-gap brainstorms point to a gap beyond run storage: users need to know whether valid-looking results can be compared across setup, sample, calibration, generated protocol, correction, and analysis differences. | Evidence and gap review only; no automatic scientific equivalence judgment, generic data normalization, software-proof of physical truth, authoritative setup model, universal schema, or full campaign workflow. |
...
- Let foundational pains drive acceptance; let adoption blockers constrain or
  invalidate a journey; convert JTBD candidates into journey phrasing; require
...
- Which `PN` rows should be converted into explicit JTBD statements in the
  ...
  acceptance detail?
- Which blind-persona adoption blockers need first-journey checks: truth drift,
  false confidence, control-PC fragility, known-good reference, rollback
...
- Can Scopecat produce useful full-fidelity internal diagnostics while still
  offering sanitized exports for public docs, external support, or other
  boundary crossings, without turning into a ticketing or remote-support
  product?
...
not to adoption blockers, cross-lab demand, role behavior, or market
