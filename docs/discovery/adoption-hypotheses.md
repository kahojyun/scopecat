...
| Manage calibrated parameter state | Mutable parameter files, copied seed states, trusted versus incomplete calibration state, named state lineages, purpose labels such as working point or exploration, reviewable diffs, bad states, drift queries, and run links. | Can users recover, compare, edit, and commit calibrated parameter states without Scopecat deciding hardware mutation? |
| Track setup bindings | Registry variants, wiring workbooks, generated chip/line state, LO/readout groupings, measurement run-start context, and parameter-retuning pressure after binding changes. | Can users record which sample/cooldown binding mapped logical entities to physical resources for a measurement without Scopecat owning hardware control? |
| Compare against selected reference context | Setup reality, selected references, user marks, support packages, preview compatibility, and control-PC safety. | Can users see changed, missing, unverified, redacted, unlinked, same-observed, or not-compared recorded context against a selected reference without Scopecat interpreting the result for them? |
