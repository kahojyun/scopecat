...
changed-observed, redacted, unavailable, and extra-file findings. It does not
accept semantic source diff, Git diagnostics, workspace mutation, dependency
...
| Editable workspace observation | Experiment code, future prepared-run pressure | A read-only observation of a selected editable folder against a managed code version. The first candidate records size and sha256 facts for observed files and reports drift or extras without accepting semantic source diff, Git diagnostics, environment readiness, import, execution, or prepared run context. |
