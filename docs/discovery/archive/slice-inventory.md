...
| `docs/discovery/slices/parameter-state/` | Adapter-authored parameter intake, storage/read views, selection, prepared-run review projections, compatibility-output candidates, and review gates. | Historical validation; retired prototype evidence. | Engineering retirement note, workflow validation map, architecture domain model. |
| `docs/discovery/slices/environment-operation/` | Modern manifest preflight, `uv sync` intent/result, and environment operation review bundles. | Historical validation; retired prototype evidence. | Workflow validation map and engineering archive notes. |
