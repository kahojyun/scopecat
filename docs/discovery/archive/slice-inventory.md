...
| `docs/discovery/slices/calibration/` | Calibration continuation, fit recovery, action recording, proposed/accepted write links, timeline, findings, and review-state projections. | Historical validation and product-candidate evidence; over-fragmented review-state slices should be summarized, not restored. | DEC-005, architecture domain model, context map, and workflow validation candidate. |
