...
| Prepared-run consumption and review chain | Promoted as manual pre-run review composition over prior read-view facts, gate facts, and scope alignment findings. | `scopecat.parameter_state.prepared_run_*` |
...
selected for prepared-run context, but that selection does not apply values to
instruments, rewrite source JSON/XLSX files, invalidate current hardware state,
or grant run-start permission.
