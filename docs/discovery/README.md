...
| Declared scan/data-shape fixtures | Measurement records support | Spike/fixtures validated | Rectangular 2D grid table, declared and observed-only ragged/adaptive table, trace-per-point table, and sidecar-declared weak 1D table pressure for preview readiness, not a final data-shape schema or importer. |
