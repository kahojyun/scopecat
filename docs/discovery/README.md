...
| Declared scan/data-shape fixtures | Measurement records support | Spike/fixtures validated | Rectangular 2D grid table, declared and observed-only ragged/adaptive table, trace-per-point table, fixed-vector response table, and sidecar-declared weak 1D table pressure for preview readiness, not a final data-shape schema or importer. |
...
| [`../../tests/fixtures/scan_data_shapes/`](../../tests/fixtures/scan_data_shapes/) | Repository-safe fixtures for declared grid, ragged, trace, fixed-vector, and sidecar-declared shape pressure. |
