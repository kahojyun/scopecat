...
This note is scoped to the first slice only. It is not a full domain model,
storage schema, API design, capability map, subsystem spec, or ADR.

## Contract Boundary

The first slice needs contracts for static explanation:
...
| Artifact role | The reason an artifact matters in the journey. | Controlled vocabulary for first slice: anchor, selected context, generated sidecar, copied snapshot, variant, code reference, setup evidence, readiness hint, unknown, fixture-authored. `Generated sidecar` is fixture evidence wording for legacy colocated artifacts; later product docs should prefer companion artifact unless a source convention specifically uses sidecar. Backup is represented as relation-level ambiguity in this slice. |
...
| Evidence view | The user-facing report produced by the slice. | Role inventory, relations, conflicts, missing facts, sharing summary, and next checks. |

## Artifact Role Vocabulary

The first slice should use a small role vocabulary:
...
The first slice should support only these relation types:
...
- the slice must not execute, import, test, install, or rewrite code;
...
The slice output should be a static evidence report with these sections:
...
[`../prototypes/static-analysis-spike.md`](../prototypes/static-analysis-spike.md) validates
...
- Is `evidence handling` the same concept as confidence, or should confidence
  remain purely narrative for this slice?
