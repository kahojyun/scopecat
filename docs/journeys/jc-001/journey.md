...
Ready as the `JC-001` journey; used by provisional design-pressure extraction
and the accepted passive evidence-view decision.

## Purpose

Define the first current-state and future-state journey for `JC-001`:
...
These were extraction prompts, not accepted capability documents,
...
[`contracts/design-pressure-ownership.md`](contracts/design-pressure-ownership.md).
