...
| Inspect running or partial measurements | Live plotters, partial rows, latest sweep state, interrupted scans, operator stop decisions. | Measurement Record, Normalized Primary Data, Review Summary, Operation Result, Durable Audit Record. | Keep slices that prove lifecycle/progress observation and partial-data review. Archive static projections that do not connect to emitted events or user decisions. |
| Continue calibration work | Failed fits, manual refits, notebooks, proposed parameter changes, blocked downstream steps. | Calibration Continuation State, Operator Acknowledgement, Context Link, Parameter State Record, Operation Result, Durable Audit Record. | Keep slices that prove accepted write handoff, action recording, and continuity. Archive over-fragmented review-state projections after extracting brownfield terms and transition rules. |
...
| Slice-to-slice glue | No | Exists mainly because one validation slice needed a durable-looking output for another slice to read; reclassify as operation result, durable audit record, review summary, manifest, or historical validation before keeping. |
