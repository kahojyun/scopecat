...
| `progressive-adoption-progress-tracker.md` | Active tracker | Tracking journey-first discovery, adoption ladders, migration wedges, contracts, and decision promotion. | `JC-001` is ready for W3 capability/adoption extraction, but W3+ ladders, wedges, capability names, and contract ideas are unordered hypotheses until promoted by the journey. |
...
| `jc-001-work-bundle-explanation-journey.md` | Ready for W3 capability/adoption extraction | First W2 journey: current-state and future-state offline explanation of an existing work bundle with selected context, code-shape evidence, generated/copied sidecars, variants, ambiguity, and sharing boundaries. | Use this as the next input for touched-capability and standalone-adoption extraction. It does not accept execution, hardware control, write-back, rollback, environment mutation, known-good comparison, or scientific-equivalence scoring. |
