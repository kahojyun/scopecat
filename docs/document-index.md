...
| `jc-001-passive-evidence-view-prototype-scope.md` | Hardened; continue as prototype script | Implementation-facing scope for the accepted passive evidence-view prototype, now recording the hardened project-owned prototype script, public-safe fixture, tests, validation command, expected-shape snapshot, acceptance checks, non-goals, risks, selected fixture strategy, and keep-as-prototype decision. | This is not implementation architecture. The first prototype remains read-only and fixture-sized; do not promote package layout, tooling, parser framework, storage, UI, fixture import, or sample-data policy from this spike alone. |
