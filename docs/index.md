...
| [`strategy/adoption-routes.md`](strategy/adoption-routes.md) | Provisional route owner | Product-value route hypotheses that may help `JC` selection compose without becoming subsystem names. | Not a roadmap, capability map, implementation order, or product acceptance claim. |
...
| [`evidence/inventory.md`](evidence/inventory.md) | Evidence owner | Canonical evidence, lab/user context, top-level pains, pain/pressure rows, anti-patterns, and `JC` candidate rows. | Interpretation rules belong in `evidence/method.md`; external references belong in `evidence/external-baseline.md`; future `JC` process guidance belongs in the operating standard. |
| [`evidence/method.md`](evidence/method.md) | Evidence method | Source confidence, bias correction, source-handling guardrails, prompt-method hygiene, design pressure handling, and external baseline interpretation. | Does not own evidence rows or journey scope. |
