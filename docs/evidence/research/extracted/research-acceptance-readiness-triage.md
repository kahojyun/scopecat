...
summarizes the evidence, pain, guardrail, baseline, and source-hygiene
claims from this triage for source-map and journey-selection work.
...
Delete or archive this note after the first journey is selected and validated
...
entry point once current docs can explain the accepted evidence and current
questions directly.
...
| The first concrete journey should likely include ordinary Python writing durable data, simple live inspection, checkpoint-safe readability, and stable-ID reopen. | Fricon interview evidence and the current progress tracker both name this as a candidate slice. | Journey selection note before concept acceptance. |
...
3. Decide whether the write/watch/reopen slice is accepted, rejected, or kept
   as one candidate among several.
4. Promote only the capabilities touched by that journey into adoption-ladder
   or capability notes.
5. Capture minimal domain concepts and contracts for the selected slice.
