...
| JC-002 | 2 | Selected high-value runs to analysis handoff package | User refinement raises internal analysis handoff above cross-machine scientific comparison as a nearer adoption pressure. This tests PN-004 with EV-044 and EV-045: users need to find high-value runs, inspect simple historical previews, mark or multi-select them, and move an immutable pre-analysis data-plus-context snapshot from an experiment-control computer to a personal analysis computer without losing source identity. | Candidate provenance only. Current draft boundary is owned by [`../journeys/jc-002/snapshot-boundary.md`](../journeys/jc-002/snapshot-boundary.md). |
...
| JC-016 | 16 | One reviewed package to one lab-owned runtime for one bounded local run | Derived boundary candidate, not independently validated. It tests the bridge from `JC-007`, `JC-008`, `JC-003`, and `JC-011` into bounded real automation using PN-017, PN-018, PN-033, TP-010, and the control-PC safety guardrails: a frozen plan, proposal, or queue package with runtime owner, supplied bounds, readiness evidence, review gates, lifecycle, stop behavior, failure policy, and audit records can be handed to one lab-owned runtime for one bounded local run. | First bounded runtime-handoff boundary only; no Scopecat-owned instrument runtime by default, generic scheduler, workflow DAG, environment sync, package deployment, broad resource-locking system, rollback guarantee, or autonomous calibration platform. |
