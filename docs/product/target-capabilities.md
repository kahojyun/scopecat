...
| CAP-003 | Parameter State Review | Discovery | Retired prototype evidence and problem-brief evidence | Restart from a named brownfield entrypoint before promoting implementation again; decide whether reviewed parameter snapshots, compatibility-file writing, hardware apply, catalog discovery, automatic run start, or a shared parameter/run-context schema is justified by a workflow. |
| CAP-004 | Environment Operation | Engineering prototype | Frozen bounded operation evidence | Keep the current `uv sync` plus optional interpreter fact-probe boundary sealed; expand runtime readiness, manifest integration, manager expansion, execution hardening, or GUI/runtime monitoring only after a named journey requires it. |
