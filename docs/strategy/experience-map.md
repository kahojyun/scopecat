...
| Analysis handoff | `JC-002` | Move selected runs and enough context to an analysis computer as a portable pre-analysis snapshot, with export/import or optional shared-storage discovery as the first cross-machine answer; finalization and integrity are separate validation questions. |
...
| Code, method, and queue readiness | `JC-004`, `JC-007`, `JC-008`, `JC-013` | Separate copied-code selection and recovery, one-time migration from ongoing checkpoint/sync pressure, frozen plan or queue intent, dry-run or mock-queue readiness, simulated or lab-owned grouped-calibration outcomes, and shared asset drift diagnostics. |
...
| Quarantined runtime-boundary question | `JC-016` | Keep the possible bridge from reviewed intent, local execution evidence, parameter memory, code-version selection, and advisory evidence visible only as a composition risk; do not treat it as route membership, prototype scope, or accepted runtime ownership. |
