...
| One station has multiple computers but one data bottleneck. | Explore same-LAN historical-only browsing -> expose historical records and selected-run previews. | Historical browsing is the minimum valuable slice; live observation belongs to later read/monitor or remote-execution validation unless it proves separate adoption payoff. |
...
| Same-station data access | `JC-017` | User-clarified candidate | Place same-LAN historical-only browsing before live observation, remote experiment execution, network-service architecture, or multi-user control semantics. |
