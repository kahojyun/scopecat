...
| One station has multiple computers but one data bottleneck. | Explore same-LAN read-only browsing -> expose historical records and previews -> optionally observe running-run progress. | Browsing is separate from remote experiment execution, command arbitration, and hardware control. |
...
| Same-station data access | `JC-017` | User-clarified candidate | Place same-LAN historical browsing and optional running-run observation before remote experiment execution, network-service architecture, or multi-user control semantics. |
