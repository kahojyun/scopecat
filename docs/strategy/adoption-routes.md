...
acceptance constraint on run explanation, analysis handoff, and durable reopen:
...
- `JC-008` should validate helper-authored batch intent, simulated or
  ...
  transcript for bounded grouped calibration if that remains necessary. The
  adoption-route question is whether rewriting the relevant experiment or
  calibration code around this route pays back enough in clearer operation,
  review, and recovery. A run-to-completion executor is the baseline; stronger
  validation should test resume, retry, review continuation, or selected
  remeasurement. It is not a managed runner.
- `JC-003` should start from parameter memory, drift, branches or working
  points, run linkage, direct-update checkpoints, and bad-state handling. Bad
  states should be kept by default with yank/exclusion-style semantics; hard
  delete is a cleanup path, not the default history model. The write/apply
  distinction and explicit proposal/review are optional future scope until they
  prove adoption payoff beyond parameter-memory checkpoints.
