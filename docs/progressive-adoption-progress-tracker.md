...
to find high-value runs on an experiment-control computer, inspect quick
previews, star or select them, and copy self-contained data-plus-context
snapshots to an analysis computer without losing source identity.
Experiment real-time visualization, durable measurement records, and existing
measurement-framework baselines should be handled as baseline-capability or
journey pressure unless they directly falsify the passive evidence-view
boundary accepted in `jc-001/`; live preview remains important adoption
pressure, but should not become a write-acknowledgement path or full analysis
viewer by default.
Predecessor Fricon data-shape work should be reused as validation coverage for
analysis handoff and durable-record journeys. It should not be promoted here
into API signatures, package manifest fields, storage layout, viewer behavior,
or framework-specific export adapters.
Analysis handoff can validate historical plotting and read readiness, but it
does not settle live-preview monitor semantics. A live-inspection journey would
still need runtime hints for active output, active axes, selected channel or
trace, preferred line/heatmap/trace/IQ view, and stale, lagging, disconnected,
or incomplete-view state.
...
| Starred runs to analysis handoff package | A user searches free-form experiment names, uses quick previews and star/favorite marks to find high-value runs, multi-selects runs like files, and creates an immutable self-contained handoff snapshot with data, source IDs, read API guidance, context evidence, fit/figure references where available, explicit missing-context warnings, and evidence-handling labels. | Measurement History, Analysis and claim lineage, Parameter Memory, Code Asset Registry | Test the adoption-critical path from experiment-control computer to personal analysis computer and validate important Fricon data-shape cases for historical plotting/read readiness without requiring generic export formats first, full work-bundle export/import, managed script execution, a permission system, publication workflow scope, live-monitor semantics, or premature reader/API/storage/UI specification. | Not Started |
