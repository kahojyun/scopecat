...
| `RunRequest` | User request snapshot for one run or segment: inputs, config source, overrides with provenance, sweeps, seeds, extra axes, extra records, execution flags, and operator metadata. |
...
| `RunScope` | Legacy capture boundary for notebooks or scripts outside structured execution. It records auditable evidence but does not imply reproducible execution. |
...
- imports from CSV, XLSX, JSON, registry trees, private runners, and external
  systems;
- instrument-driver integration and legacy runner translation;
