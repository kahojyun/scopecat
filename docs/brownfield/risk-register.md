...
| BR-RISK-007 | Operator workflow mismatch | Product adoption | Medium | Prepare a manual run, calibration continuation, running monitor, reference-based rerun, post-run results review. | ADR-0004 | [`current-state-assessment.md`](current-state-assessment.md), [`pain-points.md`](pain-points.md) | Treat current state and pain points as sampled evidence; reopen the relevant `BR-PAIN-*` row when real operator behavior does not match the modeled journey. |
