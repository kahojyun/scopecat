...
| JNY-001-SMOKE | Share selected measurement vertical-slice smoke path | JNY-001 | Production vertical slice | CAP-001, CAP-002 | Validate the workflow backbone across use-case segments: selected stored-record export, safe zip archive creation from the DEC-010 package of record, safe zip archive materialization back into the DEC-010 package of record, UC-004 read-only receiving review, import plan, and new-record durable import into a second storage root. | GUI-owned persisted receiving state, archive-backed import beyond DEC-021 and DEC-024, batch durable import beyond DEC-017, linked-context payload import beyond DEC-016, existing-record update/import beyond DEC-025, post-run browser/plotter/readiness review beyond JNY-008, and shared domain schema remain unvalidated. | Use this row to decide whether to continue cross-segment production hardening or branch into a named product fork. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Bounded `uv sync` operation evidence | Operation | JNY-002; JNY-009 | Retired prototype evidence | Historical prototype evidence validated a bounded `uv sync` intent/result/review chain with optional interpreter fact probe. | No live environment-operation owner remains. Runtime readiness, package-state truth, selected-code execution, notebook execution, hardware/service probing, multi-manager abstraction, cancellation UI, and portable projection of local paths remain unvalidated. | Restart from a named brownfield entrypoint before promoting environment-operation implementation again. |
