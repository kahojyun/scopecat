...
| --- | --- | --- | --- | --- |
| UC-CAND-002 | Discovery | Prepared-run scenario evidence exists for context construction, review gate, acknowledgement-aware review, view-state projection, setup-binding pressure, and historical environment-operation input. | No live prepared-run route owner owns explicit command/result inputs, acknowledgement or deferral, no-run-start semantics, or a receipt shape that a later Measurement Record can reference. | Does the user-facing use case have explicit input, review, acknowledgement or deferral, and no-run-start semantics? |
| UC-CAND-003 | Discovery | Experiment-code discovery and implementation-candidate evidence show pressure around recording, comparing, materializing, observing editable folders, preparing reruns, and GUI review. | No single promoted step has an independently useful user goal or live product owner. | Which one experiment-code step should be promoted first? |
| UC-CAND-004 | Discovery | Running measurement monitoring discovery evidence and related Measurement Records inspection pressure exist. | No live event route proves lifecycle, progress, and partial-data recording from Python-driven measurements without scan control. | Can Python-driven measurements emit reviewable lifecycle, progress, and partial-data events without granting scan control? |
| UC-CAND-005 | Discovery | Calibration continuation scenario evidence validates review surfaces, action recording, fit recovery, continuation composition, and parameter-state handoff pressure. | No live owner-native workflow entrypoint owns explicit user actions, review state transitions, or production-shaped acceptance criteria. | Do repeated calibration continuation cases require stable review state, continuation actions, and support expectations beyond one scenario? |
| UC-CAND-006 | Discovery | Selected-reference comparison discovery evidence validates pressure to compare declared context facts and selected code context. | No live route owns comparison findings, reference selection, setup truth non-claims, or action handoff. | Can Scopecat compare declared context facts and selected code context without claiming setup truth or domain judgment? |
| UC-CAND-007 | Discovery | Measurement Records browser/plotter pressure, review/read-model evidence, and selected-record export freshness evidence exist. | No live post-run route owns records browsing, plotting, readiness review, operator notes, or review state. | Which first route should own records browser, plotter, and readiness-review behavior before handoff, comparison, calibration continuation, or rerun preparation? |
...
| Scenario | Supports | Evidence State | Use For | Promotion Rule |
| --- | --- | --- | --- | --- |
| Calibration continuation review | UC-CAND-005, JNY-003 | Discovery evidence | Review surface, action recording, fit recovery, continuation composition, and parameter-state handoff pressure. | Promote only if a narrow calibration continuation use case has user-step acceptance criteria. |
| Prepared run context and acknowledgement | UC-CAND-002, JNY-002 | Discovery evidence | Context construction, review gate, acknowledgement-aware review, view-state projection, setup-binding pressure, and historical environment-operation pressure. | Promote only around a prepared-run use case with command/result model and acceptance criteria. |
