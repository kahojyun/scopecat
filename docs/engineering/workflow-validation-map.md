...
persisted GUI/batch/context/archive/signature contract, or a different
journey's first live route.
...
| UC-006 | Selected stored Measurement Record becomes a handoff package | JNY-001 | Production vertical slice candidate | CAP-001, CAP-002 | Export one or more complete stored Measurement Records through the handoff package writer; read each record-local read model, creation manifest, and writer receipt; preserve durable record identity, label, experiment type, primary-data digest/size/path continuity, and package-relative primary-data topology; require explicit preview metadata; optionally package explicitly declared record-local linked-context payloads under `context/` without treating recorded references as file-copy authority; produce an openable package without mutating record storage; validate the workflow backbone through source storage, export, read-only receiving review, import plan, and durable import into a second storage root; cover stale/missing source evidence, no-overwrite package collision, blocked-export review guidance, corrupted package bytes, receiving/import-plan review guidance, receiving review mismatch, durable-import review guidance, and retry-summary behavior; use the DEC-010 directory manifest package format while archive creation/extraction is deferred under DEC-020; follow DEC-011 by treating declared digest integrity as local-review evidence without claiming signature validation, authenticity, sender trust, or scientific validity; keep linked-context payload import deferred under DEC-016, batch durable import deferred under DEC-017, receiving review state as a local projection under DEC-018, and signature/trust implementation deferred under DEC-019. | Read-model refresh, existing-record export/update policy, persisted GUI state, signed package/trusted-source policy beyond DEC-019, archive extraction beyond DEC-020, and shared domain schema remain unvalidated. | Decide whether to continue production hardening, persisted GUI state, signed package policy, archive materialization, or a separate handoff extension. |
