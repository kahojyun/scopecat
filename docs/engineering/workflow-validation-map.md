...
| UC-006 | Selected stored Measurement Record becomes a handoff package | JNY-001 | Production vertical slice | CAP-001, CAP-002 | Export one complete stored Measurement Record through the handoff package writer; read the record-local read model, creation manifest, and writer receipt; preserve durable record identity, label, experiment type, primary-data digest/size/path continuity, and package-relative primary-data topology; require explicit preview metadata; optionally package explicitly declared record-local linked-context payloads under `context/` without treating recorded references as file-copy authority; produce an openable package without mutating record storage; validate the workflow backbone through source storage, export, safe zip archive materialization into the DEC-010 directory package of record, read-only receiving review, import plan, and durable import into a second storage root; cover stale/missing source evidence, no-overwrite package collision, blocked-export review guidance, read-model freshness review without refresh or storage mutation, user-transparent pre-export read-model refresh composition through the Measurement Records refresh route, corrupted package bytes, receiving/import-plan review guidance, receiving review mismatch, durable-import review guidance, and retry-summary behavior; use the DEC-010 directory manifest package format as the package of record with DEC-021 zip transport materialization; follow DEC-011 by treating declared digest integrity as local-review evidence without claiming signature validation, authenticity, sender trust, or scientific validity; follow DEC-022 by treating the future signed scope as the DEC-010 package of record rather than archive bytes; keep linked-context payload import deferred under DEC-016, batch durable import deferred under DEC-017, receiving review state as a local projection under DEC-018, signature/trust implementation deferred under DEC-019, and archive creation/archive-backed import outside DEC-021. | Existing-record export/update policy, persisted GUI state, source-side batch export productization beyond DEC-015, signature verification/trusted-source implementation beyond DEC-019 and DEC-022, archive creation/archive-backed import beyond DEC-021, and shared domain schema remain unvalidated. | Decide whether to continue production hardening, persisted GUI state, signed package verification/trust policy, archive creation, batch export productization, or a separate handoff extension. |
