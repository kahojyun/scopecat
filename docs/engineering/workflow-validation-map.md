...
questions are GUI receiving state, signature/trust implementation beyond
DEC-011, archive packaging/extraction, or a different journey's first live
route.
...
| UC-004 | Handoff package is received and imported into local storage | JNY-001 | Engineering prototype | CAP-002, CAP-001 | Review a handoff package on a receiving side; open package read-only; observe manifest-declared integrity; run receiving gate; create a non-mutating import plan for one or more package measurements; keep linked-context payloads as reviewable package contents without durable materialization under DEC-016; keep batch durable import deferred under DEC-017; adapt exactly one ready planned measurement into Measurement Records durable import; summarize import receipt and retry reasonableness. | Archive extraction/trust, signatures/authenticity beyond DEC-011, conflict resolution beyond delegated durable import rules, and persistent receiving review state remain unvalidated. | Decide separately if GUI receiving state, signature/trust, archive extraction/trust, or another receiving extension is the next user risk. |
