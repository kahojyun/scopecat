...
| UC-006 | Selected stored Measurement Record becomes a handoff package | JNY-001 | Production vertical slice segment | CAP-001, CAP-002 | Export one complete stored Measurement Record through the handoff package writer; read the record-local read model, creation manifest, and writer receipt; preserve durable record identity, label, experiment type, primary-data digest/size/path continuity, and package-relative primary-data topology; require explicit preview metadata; optionally package explicitly declared record-local linked-context payloads under `context/` without treating recorded references as file-copy authority; produce an openable package without mutating record storage; cover stale/missing source evidence, no-overwrite package collision, blocked-export review guidance, read-model freshness review without refresh or storage mutation, and user-transparent pre-export read-model refresh composition through the Measurement Records refresh route; follow DEC-011 by treating declared digest integrity as local-review evidence without claiming external authenticity, sender trust, or scientific validity; keep source storage mutation, receiving review, import planning, receiving review-state materialization, durable import, archive-backed import, existing-record update, and final storage schema outside this use-case segment. | Receiving/import behavior belongs to UC-004 and UC-002; source-side batch export productization beyond DEC-015, durable linked-context payload import beyond DEC-016, existing-record update/import beyond DEC-025, and shared domain schema remain unvalidated. | Decide separately whether source-side selected export needs more production hardening, batch export productization, or linked-context payload follow-up. |
| JNY-001-SMOKE | Portable measurement handoff vertical-slice smoke path | JNY-001 | Production vertical slice | CAP-001, CAP-002 | Validate the workflow backbone across use-case segments: source storage, UC-006 selected export, safe zip archive creation from the DEC-010 package of record, safe zip archive materialization back into the DEC-010 package of record, UC-004 read-only receiving review, import plan, local receiving review-state receipt materialization, and UC-002 durable new-record import into a second storage root. The smoke summary records stage classifications, final operator result, source-storage-read-only export, transport-only archive bytes, materialized DEC-010 package authority, new-record durable import, and existing-record update non-claim. | GUI-owned persisted receiving state beyond DEC-018 and DEC-023, archive-backed import beyond DEC-021 and DEC-024, batch durable import beyond DEC-017, linked-context payload import beyond DEC-016, existing-record update/import beyond DEC-025, and shared domain schema remain unvalidated. | Use this row to decide whether to continue cross-segment production hardening or branch into a named product fork. |
