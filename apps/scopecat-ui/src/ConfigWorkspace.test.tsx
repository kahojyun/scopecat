...
import { getRunParameterProposals } from "./proposal-api";
...
vi.mock("./proposal-api", async (importOriginal) => ({
  ...(await importOriginal<typeof import("./proposal-api")>()),
...
describe("ConfigWorkspace", () => {
  it("presents saved versions as defaults and undo without generation ceremony", async () => {
    ...
    expect(
      screen.queryByText("Runtime-derived default"),
    ).not.toBeInTheDocument();
    expect(
      await screen.findByText("Direct configuration profile"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Saved from one complete config snapshot."),
    ).toBeInTheDocument();
    expect(screen.queryByText(/application-provided/i)).not.toBeInTheDocument();

    fireEvent.click(
      screen.getByRole("button", { name: /calibrated.*direct profile/i }),
    );
    fireEvent.click(
    ...
    expect(window.confirm).toHaveBeenCalledWith(
      "Set calibrated as the default configuration?",
    );
    ...
    expect(window.confirm).toHaveBeenLastCalledWith(
      "Restore calibrated as the default configuration?",
    );
  });
  ...
  it.each(["manual_parameter_updates", "candidate_config"] as const)(
    ...
    async (sourceKind) => {
      ...
      expect(
        within(notice).getByText("Runtime-derived default"),
      ).toBeInTheDocument();
  ...
  it("opens the immutable base entry for a manual parameter update", async () => {
    ...
    expect(
      await screen.findByText("Manual parameter update"),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", { name: "Open base version baseline" }),
    );
    ...
    expect(
      screen.getByText("Direct configuration profile"),
    ).toBeInTheDocument();
  });
  ...
  it("joins candidate proposals, analyses, and the latest decision", async () => {
    ...
    expect(
      within(gainEvidence).getByText("Approved · Human · Grace"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(
        "Approved · Automatic policy fit-confidence@2 · nightly-calibration",
      ),
    ).toBeInTheDocument();
    expect(screen.getByText("High-confidence fit")).toBeInTheDocument();
    expect(screen.queryByText("Older note")).not.toBeInTheDocument();
    expect(
      document.querySelector('time[datetime="2026-07-24T08:00:00Z"]'),
    ).toBeInTheDocument();

    fireEvent.click(
      screen.getByRole("button", { name: "Open producing run" }),
    );
  ...
  it("keeps decision note and time unresolved while proposals load", async () => {
    ...
    vi.mocked(getRunParameterProposals).mockImplementation(
      () =>
        new Promise<
          Awaited<ReturnType<typeof getRunParameterProposals>>
        >(() => undefined),
    );
  ...
  it("does not report missing decision note or time when proposals fail", async () => {
    ...
    vi.mocked(getConfigRegistryEntry).mockResolvedValue(entryDetail(entry));
    vi.mocked(getRunParameterProposals).mockRejectedValue(
      new Error("proposal store offline"),
    );
    ...
    expect(
      await screen.findByText(
        "Proposal evidence unavailable: proposal store offline",
      ),
    ).toBeInTheDocument();
    ...
    expect(
      within(evidence).getByText("Proposal details unavailable"),
    ).toBeInTheDocument();
    expect(
      within(evidence).getByText("Decision unavailable"),
    ).toBeInTheDocument();
    expect(within(evidence).queryByText("Note")).not.toBeInTheDocument();
    expect(within(evidence).queryByText("Decided")).not.toBeInTheDocument();
  });
