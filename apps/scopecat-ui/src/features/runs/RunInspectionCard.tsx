    ...
    <article className={classes(detailCard, "col-span-full")} data-testid="run-inspection-card">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-text-soft">
          ...
          <strong className="text-[0.76rem]">Compiled point inspection</strong>
        </div>
              ...
              <div className="mb-3 flex flex-wrap items-start justify-between gap-3 rounded-md border border-line bg-panel p-3">
                <div>
                  <strong className="text-[0.7rem]">
                    {selected.outcome === "accepted" && selected.accepted_point
                      ? `Run point #${selected.accepted_point.point_index! + 1}`
                      : "Rejected candidate"}
                  </strong>
                  <div className="mt-1 text-[0.62rem] text-text-dim">
                    {formatInspectionCoordinates(selected.candidate.coordinates)}
                  </div>
                  {selected.reason && (
...
function EventStatus({
  ...
  const pointIndex = event.accepted_point?.point_index;
  const completed =
    pointIndex !== null && pointIndex !== undefined && pointIndex < completedPointCount;
  return completed ? (
