"""Structured summary builder for reference-based rerun preparation.
...
bindings, sync environments, import code, execute code, correct drift, infer
cause, guarantee reproducibility, or define a universal context schema.
"""
...
def _validate_target_matches_measurement_intent(
    ...
    for field in compared_fields:
        if field not in target or field not in intent:
            raise ValueError(
                f"proposed run target and selected measurement intent require field: {field}"
            )
        if target.get(field) != intent.get(field):
            raise ValueError(
                f"proposed run target does not match selected measurement intent field: {field}"
            )
...
def build_reference_based_rerun_preparation_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Build a reference-based rerun preparation summary from explicit fixture input."""
