...
This candidate opens a Scopecat-authored handoff package enough for immediate
SDK/GUI use: manifest orientation, primary CSV loading, and declared-preview
plot data. It deliberately does not accept/import the package, mutate storage,
extract archives, claim package integrity, infer schemas, or define GUI
behavior.
"""
    ...
    return [
        {
            ...
            "severity": "info",
            "basis": "Declared package manifest and package-local primary data were opened without storage mutation.",
            "does_not_claim": "package_acceptance_or_import",
        },
        {
            "code": "package_integrity_not_claimed",
            "severity": "review",
            "basis": "Declared digest and size facts remain manifest facts where present; this opener does not compare them.",
...
def open_handoff_package(package_dir: Path) -> dict[str, Any]:
    ...
    return {
        "package_open_policy": copy.deepcopy(_EXPECTED_POLICY),
        "package": {
            "package_id": manifest["package_identity"]["package_id"],
