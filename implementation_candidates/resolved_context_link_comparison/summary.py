"""Structured summary builder for selected-reference resolved context links.

This module is an experimental production-shaped boundary. It compares
explicit measurement-record context links only and deliberately avoids
measurement intent selectors, raw data, fit quality, context payloads, hardware
state, readiness claims, restore behavior, code import, and execution.
"""
...
def build_resolved_context_link_comparison_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Build selected-reference comparison findings from resolved context links."""
    _validate_references(source)
    request = _validate_request(source)
    return {
