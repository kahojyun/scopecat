...
class SourceAgnosticParameterStateReadRequest:
    """Typed route-local request for explicit source-agnostic state reads."""
    ...
    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> SourceAgnosticParameterStateReadRequest:
    ...
    def request_ids(self) -> tuple[str, ...]:
        return tuple(request["request_id"] for request in self._source["read_requests"])


@dataclass(frozen=True, init=False)
class SourceAgnosticParameterStateReadResult:
    """Typed route-local result for source-agnostic parameter-state reads."""
    ...
    @property
    def stored_states(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["stored_states"])
...
def read_source_agnostic_parameter_state_view(
    ...
    *,
    ...
    request_model = SourceAgnosticParameterStateReadRequest.from_dict(source)
