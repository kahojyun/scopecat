...
class HandoffSnapshot:
    ...
    def _validate_run(self, run: dict[str, Any]) -> None:
        ...
        if run.get("primary_artifact_id") not in self.artifacts_by_id:
...
def render_svg_plot(group: dict[str, Any], output_path: Path, run_id: str | None = None) -> None:
    ...
    height = 360
    margin = 48
    all_x = [row["frequency_hz"] for run in selected_runs for row in run["data"]]
    all_y = [row["response_v"] for run in selected_runs for row in run["data"]]
    ...
    def point(row: dict[str, float]) -> tuple[float, float]:
        x = margin + (row["frequency_hz"] - min_x) / (max_x - min_x) * (width - 2 * margin)
        y = height - margin - (row["response_v"] - min_y) / (max_y - min_y) * (height - 2 * margin)
        return x, y
    ...
    lines = [
        ...
        '<text x="300" y="346" font-family="sans-serif" font-size="12">frequency (Hz)</text>',
        (
            '<text x="8" y="190" font-family="sans-serif" font-size="12" '
            'transform="rotate(-90 8 190)">response (V)</text>'
        ),
    ]
