...
from quantum_lab_demo.experiments.ids import QND_REPEATED_MEASUREMENT_TEMPLATE_ID
...
def QND_REPEATED_MEASUREMENT_TEMPLATE(
    ...
    call = qnd_repeated_measurement_program(
        qubit=qubit,
        rounds=rounds,
        shots=shots,
    )
    return sc.experiment(call).record_product(call.results.qnd_iq, record_id="qnd_iq")
