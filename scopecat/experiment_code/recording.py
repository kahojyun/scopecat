"""Experiment-code recording engineering prototype.

This route-local module is deliberately side-effect free: it does not read
source files, inspect Git state, scan unrecorded folders, discover
dependencies, import code, execute code, restore environments, materialize
...
class ExperimentCodeRecordingRequest:
    """Typed local request for explicit experiment-code recording facts."""
    ...
    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> ExperimentCodeRecordingRequest:
...
@dataclass(frozen=True, init=False)
class ExperimentCodeRecordingResult:
    """Experiment-code recording summary projection."""
    ...
    @property
    def recorded_code_contexts(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["recorded_code_contexts"])

    @property
    def code_snapshot_records(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["code_snapshot_records"])

    def to_dict(self) -> dict[str, Any]:
        return copy.deepcopy(self._summary)


def summarize_experiment_code_recording(
    request: ExperimentCodeRecordingRequest,
) -> ExperimentCodeRecordingResult:
    """Summarize explicit experiment-code recording facts."""
    ...
    return ExperimentCodeRecordingResult(summary=summary)


def build_experiment_code_recording_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Raw-dictionary adapter for experiment-code recording fixtures."""
    return summarize_experiment_code_recording(
        ExperimentCodeRecordingRequest.from_dict(source)
    ).to_dict()
