"""Prepared-run context construction engineering prototype.

This module composes declared family-owned context records into prepared-run
context summaries. It deliberately does not inspect workspaces, control
hardware, write parameters, mutate setup bindings, sync environments, import
code, execute code, restore context, or define a universal context schema.
"""
...
EXPECTED_PREPARED_RUN_CONTEXT_POLICY = {
...
ALLOWED_INCLUDE_STATES = {
    "selected",
    "unavailable",
    "optional_not_selected",
}

OBSERVATION_FINDINGS_NEEDING_REVIEW = {
    "changed_observed",
    "missing_expected",
    "target_is_symlink",
    "not_a_file",
...
class PreparedRunContextRequest:
    """Typed local request over declared prepared-run context records."""
    ...
    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> PreparedRunContextRequest:
...
@dataclass(frozen=True, init=False)
class PreparedRunContextResult:
    """Prepared-run context summary projection."""
    ...
    @property
    def prepared_run_contexts(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["prepared_run_contexts"])

    @property
    def missing_context_findings(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["missing_context_findings"])

    @property
    def workspace_context_findings(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["workspace_context_findings"])

    def to_dict(self) -> dict[str, Any]:
        return copy.deepcopy(self._summary)


def compose_prepared_run_context(
    request: PreparedRunContextRequest,
) -> PreparedRunContextResult:
    """Compose declared family-owned context records into prepared-run summaries."""
    ...
    return PreparedRunContextResult(summary=summary)


def build_prepared_run_context_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Raw-dictionary adapter for prepared-run context construction."""

    request = PreparedRunContextRequest.from_dict(source)
    return compose_prepared_run_context(request).to_dict()
...
def _validate_policy(source: dict[str, Any]) -> None:
    ...
    if set(policy) != set(EXPECTED_PREPARED_RUN_CONTEXT_POLICY):
        raise ValueError("prepared run context policy must match the expected policy shape")
    for key, expected in EXPECTED_PREPARED_RUN_CONTEXT_POLICY.items():
...
def _validate_selected_context(
    ...
    if include_state not in ALLOWED_INCLUDE_STATES:
...
def _validate_measurement_intent_alignment(
    ...
    for compared_field in compared_fields:
        if compared_field not in target or compared_field not in intent:
            raise ValueError(
                f"manual run target and selected measurement intent require field: {compared_field}"
            )
        if target.get(compared_field) != intent.get(compared_field):
            raise ValueError(
                "manual run target does not match selected measurement intent field: "
                f"{compared_field}"
            )
...
def _workspace_context_findings(
    ...
    for prepared_context in source["prepared_run_contexts"]:
        ...
        review_findings = {
            ...
            if finding in OBSERVATION_FINDINGS_NEEDING_REVIEW and count > 0
        }
        if review_findings:
            findings.append(
                {
