"""Selected-reference comparison engineering prototype.

This module compares explicit fixture facts only. It deliberately does not
inspect raw measurement data, source files, Git state, runtime environments,
hardware state, or user-authored analysis conclusions.
"""
...
_ComparisonMode = Literal["basic_context", "code_context"]


@dataclass(frozen=True, init=False)
class SelectedReferenceComparisonRequest:
    """Typed local request for selected-reference context comparison."""

    _source: dict[str, Any] = field(repr=False)

    def __init__(self, *, source: dict[str, Any], mode: _ComparisonMode) -> None:
        _validate_source_for_mode(source, mode)
        object.__setattr__(self, "_source", copy.deepcopy(source))

    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> SelectedReferenceComparisonRequest:
        return cls(source=source, mode="basic_context")
...
@dataclass(frozen=True, init=False)
class SelectedReferenceCodeContextComparisonRequest:
    """Typed local request for recorded-code selected-reference comparison."""
    ...
    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> SelectedReferenceCodeContextComparisonRequest:
        return cls(source=source, mode="code_context")
...
@dataclass(frozen=True, init=False)
class SelectedReferenceComparisonResult:
    """Route-local selected-reference comparison projection."""
...
def compare_selected_reference_context(
    request: SelectedReferenceComparisonRequest,
) -> SelectedReferenceComparisonResult:
    """Compare declared current/reference context facts."""
    source = request.source
    summary = {
        "comparison": _comparison_header(source["comparison_request"]),
        "not_compared_scope": copy.deepcopy(source["comparison_request"]["not_compared_scope"]),
    ...
    return SelectedReferenceComparisonResult(summary=summary)


def build_selected_reference_context_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Raw-dictionary adapter for selected-reference context comparison."""
    request = SelectedReferenceComparisonRequest.from_dict(source)
    return compare_selected_reference_context(request).to_dict()


def compare_selected_reference_code_context(
    request: SelectedReferenceCodeContextComparisonRequest,
) -> SelectedReferenceComparisonResult:
    """Compare declared recorded-code context facts."""
    source = request.source
    summary = {
        "comparison": _comparison_header(source["comparison_request"]),
        "not_compared_scope": copy.deepcopy(source["comparison_request"]["not_compared_scope"]),
...
def build_selected_reference_code_context_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Raw-dictionary adapter for recorded-code selected-reference comparison."""
    request = SelectedReferenceCodeContextComparisonRequest.from_dict(source)
    return compare_selected_reference_code_context(request).to_dict()
...
def _validate_source_for_mode(source: dict[str, Any], mode: _ComparisonMode) -> None:
    if mode == "basic_context":
        _validate_request(
            source,
...
def _ordered_findings(
    ...
    if set(findings_by_code) != set(expected_codes):
        ...
        raise ValueError(
            f"finding codes do not match selected-reference boundary: missing={missing} "
            f"extra={extra}"
        )
