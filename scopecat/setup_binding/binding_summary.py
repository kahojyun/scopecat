...
This module validates and summarizes explicit setup-binding facts only. It
does not inspect hardware, mutate parameters, execute project generator code,
interpret user/project-defined inner payloads, define station-registry
ownership, or accept a shared snapshot framework.
"""
...
class SetupBindingSummaryRequest:
    """Typed local request for setup-binding context summary."""
    ...
    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> SetupBindingSummaryRequest:
...
@dataclass(frozen=True, init=False)
class SetupBindingSummaryResult:
    """Route-local setup-binding review projection."""
    ...
    @property
    def setup_bindings(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["setup_bindings"])

    @property
    def measurement_references(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["measurement_references"])
...
def summarize_setup_binding_context(
    request: SetupBindingSummaryRequest,
) -> SetupBindingSummaryResult:
    """Summarize declared setup-binding context facts."""
    ...
    return SetupBindingSummaryResult(summary=summary)


def build_setup_binding_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Raw-dictionary adapter for setup-binding summary."""
    request = SetupBindingSummaryRequest.from_dict(source)
    return summarize_setup_binding_context(request).to_dict()
...
def _path_is_relative(value: str) -> bool:
...
def _validate_reference_path(path: str) -> None:
    if not _path_is_relative(path):
        raise ValueError("source research notes must be repository-relative references")
...
def _validate_station_registry(registry: dict[str, Any]) -> None:
    _validate_public_safe_token(registry["registry_id"], "station registry id")
    ...
    resources = _records_by_key(registry["resource_labels"], "resource_id")
    for resource_id in resources:
        _validate_public_safe_token(resource_id, "station registry resource id")


def _validate_parameter_state(parameter_state: dict[str, Any]) -> None:
    _validate_public_safe_token(parameter_state["snapshot_id"], "parameter state snapshot id")
...
def _validate_setup_binding(
    ...
    _validate_public_safe_token(snapshot["snapshot_id"], "setup binding snapshot id")
    _validate_public_safe_token(snapshot["sample_id"], "setup binding sample id")
    _validate_public_safe_token(snapshot["cooldown_id"], "setup binding cooldown id")
    ...
    for logical_binding in snapshot["logical_bindings"]:
        _validate_public_safe_token(logical_binding["logical_entity"], "logical binding entity")
        _validate_public_safe_token(logical_binding["role"], "logical binding role")
    ...
    for view in snapshot["generated_views"]:
        _validate_public_safe_token(view["view_id"], "generated view id")
    for artifact in snapshot["source_artifacts"]:
        _validate_public_safe_token(artifact["artifact_id"], "source artifact id")
...
def _validate_diff(diff: dict[str, Any], setup_bindings: dict[str, dict[str, Any]]) -> None:
    _validate_public_safe_token(diff["diff_id"], "binding diff id")
...
def _validate_measurement(
    ...
    for input_ref in measurement["inputs"]:
        ...
        if name == "setup_binding" and snapshot_id not in setup_bindings:
            raise ValueError("measurement references missing setup binding")
        if name == "station_registry" and snapshot_id not in station_registries:
            raise ValueError("measurement references missing station registry")
        if name not in _INPUT_SNAPSHOT_LIFECYCLE:
...
def _validate_references(source: dict[str, Any]) -> None:
    if source["fixture_status"] != "synthetic_public_safe":
        raise ValueError("setup-binding fixture source must be synthetic_public_safe")
    for path in source["source_research_notes"]:
