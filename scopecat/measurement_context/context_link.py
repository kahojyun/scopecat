"""Measurement-record context link engineering prototype.

This module summarizes explicit measurement-record context links only. It does
not inspect context payloads, observe primary data, traverse relation graphs,
import context, restore state, execute code, or define a shared context schema.
"""
...
class MeasurementContextLinkRequest:
    """Typed local request for explicit measurement-record context links."""
    ...
    @classmethod
    def from_dict(cls, source: dict[str, Any]) -> MeasurementContextLinkRequest:
...
@dataclass(frozen=True, init=False)
class MeasurementContextLinkResult:
    """Route-local context-link summary projection."""
    ...
    @property
    def measurement_records(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["measurement_records"])

    @property
    def linked_context_refs(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["linked_context_refs"])

    @property
    def optional_context_findings(self) -> tuple[dict[str, Any], ...]:
        return tuple(copy.deepcopy(item) for item in self._summary["optional_context_findings"])

    def to_dict(self) -> dict[str, Any]:
        return copy.deepcopy(self._summary)


def summarize_measurement_context_links(
    request: MeasurementContextLinkRequest,
) -> MeasurementContextLinkResult:
    """Summarize explicit measurement-record context links."""
    ...
    return MeasurementContextLinkResult(summary=summary)


def build_measurement_context_link_summary(source: dict[str, Any]) -> dict[str, Any]:
    """Raw-dictionary adapter for explicit measurement-record context links."""

    request = MeasurementContextLinkRequest.from_dict(source)
    return summarize_measurement_context_links(request).to_dict()
...
def _validate_policy(source: dict[str, Any]) -> None:
    ...
    if set(policy) != set(CONTEXT_LINK_POLICY):
        raise ValueError("context link policy must match expected shape")
    for key, expected in CONTEXT_LINK_POLICY.items():
...
def _validate_context_link(
    ...
    if include_state not in INCLUDE_STATES:
...
def _validate_measurement_records(source: dict[str, Any]) -> None:
    ...
    for measurement in source["measurement_records"]:
        _validate_public_safe_token(measurement["measurement_record_id"], "measurement record id")
        seen_links = set()
...
def _attention(source: dict[str, Any]) -> list[dict[str, str]]:
    attention = [
        ...
        },
        {
            ...
            "severity": "info",
            "basis": (
                "Linked context records remain family-owned and are not imported into "
