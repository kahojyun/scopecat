...
OPERATOR_REVIEW_POLICY = {
    ...
    "context_attachment_authority": "record_local_context_attachment_receipts",
...
class MeasurementRecordOperatorReviewRun:
    ...
    context_attachment_review: dict[str, Any] | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "context_attachments": (
                copy.deepcopy(self.context_attachment_review)
                if self.context_attachment_review is not None
                else {
...
def review_measurement_records_from_request(
    ...
    *,
    ...
    context_attachment_review = list_measurement_record_context_attachments(
        storage_root=root,
        records_dir=request.records_dir,
    )
    ...
    return MeasurementRecordOperatorReviewRun(
        ...
        context_attachment_review=context_attachment_review,
...
def _aggregate_findings(
    ...
    context_attachment_review: dict[str, Any],
) -> list[dict[str, str]]:
    running_record_dirs = {
    ...
    findings.extend(
        _child_finding("context_attachment", finding)
        for finding in context_attachment_review["review_findings"]
    )
