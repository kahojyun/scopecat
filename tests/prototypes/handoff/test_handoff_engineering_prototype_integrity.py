...
from scopecat.handoff import observe_package_integrity
...
class HandoffEngineeringPrototypeIntegrityTest(unittest.TestCase):
    def test_observes_verified_declared_primary_data_integrity(self) -> None:
        ...
        member = report.member_observations[0]

        self.assertEqual(report.classification, "declared_integrity_verified")
        self.assertEqual(report.package_id, "handoff-package-legacy-rabi-001")
        self.assertEqual(report.member_count, 1)
        self.assertEqual(report.integrity_findings, ())
        ...
        self.assertEqual(
            summary["integrity_observation_policy"]["import_acceptance"],
            "not_performed",
        )
    ...
    def test_reports_changed_package_member_as_integrity_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            report = observe_package_integrity(package_dir)

        member = report.member_observations[0]
        self.assertEqual(report.classification, "integrity_review_required")
        self.assertEqual(member.comparison, "mismatch")
        self.assertEqual(member.mismatches, ("digest", "size_bytes"))
        self.assertEqual(
            report.integrity_findings[0]["finding"],
    ...
    def test_reports_missing_package_member_without_importing(self) -> None:
        ...
        self.assertEqual(report.integrity_findings[0]["finding"], "unavailable")
    ...
    def test_reports_observed_member_without_declared_integrity_facts(self) -> None:
        ...
        member = report.member_observations[0]
        self.assertEqual(report.classification, "integrity_observed_with_undeclared_members")
        self.assertEqual(member.observation_state, "observed")
    ...
    def test_observes_distinct_packaged_linked_context_integrity(self) -> None:
        ...
        observations = {
            member.package_path: member.to_dict() for member in report.member_observations
        }
        self.assertEqual(report.classification, "declared_integrity_verified")
    ...
    def test_rejects_partial_integrity_declaration_on_packaged_linked_context(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            package_dir = _copy_package(Path(temp_dir))
            manifest = _load_manifest(package_dir)
            manifest["linked_context"][0].update(
            ...
            with self.assertRaisesRegex(ValueError, "digest and size_bytes"):
