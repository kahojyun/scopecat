from __future__ import annotations

import copy
import json
import unittest
from pathlib import Path

from implementation_candidates.prepared_run_source_agnostic_parameter_state_consumption import (
    build_prepared_run_source_agnostic_parameter_state_consumption_summary,
)

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = (
    ROOT
    / "tests"
    / "fixtures"
    / "prepared_run_source_agnostic_parameter_state_consumption"
    / "basic_consumption"
)


def _load_input() -> dict:
    return json.loads((FIXTURE / "consumption-input.json").read_text(encoding="utf-8"))


def _expected_candidate() -> dict:
    return json.loads((FIXTURE / "expected-consumption-summary.json").read_text(encoding="utf-8"))[
        "candidate_summary"
    ]


def _parameter_context_ref(source: dict) -> dict:
    for ref in source["prepared_run_context_summary"]["selected_context_refs"]:
        if ref["family"] == "parameter_state" and ref["role"] == "calibrated_values":
            return ref
    raise AssertionError("fixture must include parameter_state calibrated_values ref")


class PreparedRunSourceAgnosticParameterStateConsumptionSummaryCandidateTest(unittest.TestCase):
    def test_builds_expected_summary(self) -> None:
        summary = build_prepared_run_source_agnostic_parameter_state_consumption_summary(
            _load_input()
        )

        self.assertEqual(summary, _expected_candidate())

    def test_output_does_not_alias_input_nested_objects(self) -> None:
        source = _load_input()
        summary = build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

        selected_state = source["source_agnostic_read_view_summary"]["stored_states"][1]
        selected_state["trusted_entries"][0]["value"] = "mutated"
        source["prepared_run_context_summary"]["prepared_run_contexts"][0]["label"] = "mutated"

        self.assertEqual(summary["trusted_entries"][0]["value"], 0.437)
        self.assertEqual(
            summary["prepared_run_context"]["label"],
            "qA chevron manual run context from calibration-derived parameter state",
        )

    def test_policy_must_match_expected_boundary(self) -> None:
        source = _load_input()
        source["consumption_policy"]["fresh_storage_read"] = "performed"

        with self.assertRaisesRegex(ValueError, "fresh_storage_read"):
            build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

    def test_prepared_context_must_not_claim_parameter_write_back(self) -> None:
        source = _load_input()
        source["prepared_run_context_summary"]["prepared_run_context_policy"][
            "parameter_write_back"
        ] = "performed"

        with self.assertRaisesRegex(ValueError, "parameter_write_back"):
            build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

    def test_read_view_must_not_claim_catalog_discovery_or_compat_output(self) -> None:
        source = _load_input()
        source["source_agnostic_read_view_summary"]["read_view_policy"]["catalog_discovery"] = (
            "performed"
        )

        with self.assertRaisesRegex(ValueError, "catalog_discovery"):
            build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

        source = _load_input()
        source["source_agnostic_read_view_summary"]["read_view_policy"]["compatibility_output"] = (
            "produced"
        )

        with self.assertRaisesRegex(ValueError, "compatibility_output"):
            build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

    def test_parameter_context_id_mismatch_is_review_finding(self) -> None:
        source = _load_input()
        _parameter_context_ref(source)["context_id"] = "different-state-id"
        source["consumption_request"]["parameter_context_id"] = "different-state-id"

        summary = build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

        self.assertEqual(summary["classification"], "prepared_run_parameter_state_needs_review")
        self.assertIn(
            "prepared_context_state_id_mismatch",
            {finding["code"] for finding in summary["review_findings"]},
        )

    def test_request_parameter_context_id_must_match_selected_context(self) -> None:
        source = _load_input()
        source["consumption_request"]["parameter_context_id"] = "different-state-id"

        with self.assertRaisesRegex(ValueError, "parameter_context_id"):
            build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

    def test_selected_state_read_findings_are_carried_forward(self) -> None:
        source = _load_input()
        selected_state = source["source_agnostic_read_view_summary"]["stored_states"][1]
        selected_state["classification"] = "stored_parameter_state_observed_with_mismatch"
        selected_state["review_findings"] = [
            {
                "code": "manifest_digest_mismatch",
                "severity": "review",
                "basis": "Observed manifest digest differs.",
                "does_not_claim": "repair",
            }
        ]

        summary = build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

        self.assertEqual(summary["classification"], "prepared_run_parameter_state_needs_review")
        self.assertIn(
            "selected_parameter_state_read_view_not_ready",
            {finding["code"] for finding in summary["review_findings"]},
        )
        self.assertIn(
            "selected_parameter_state_read_view_finding",
            {finding["code"] for finding in summary["review_findings"]},
        )

    def test_missing_selected_parameter_context_is_review_finding(self) -> None:
        source = _load_input()
        ref = _parameter_context_ref(source)
        ref["include_state"] = "unavailable"
        ref["context_id"] = None
        ref["missing_reason"] = "No reviewed stored parameter state was selected."

        summary = build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

        self.assertEqual(
            summary["classification"], "prepared_run_parameter_state_unavailable_for_review"
        )
        self.assertIn(
            "prepared_parameter_context_unavailable",
            {finding["code"] for finding in summary["review_findings"]},
        )

    def test_missing_expected_state_is_review_finding(self) -> None:
        source = _load_input()
        source["consumption_request"]["expected_state_id"] = "param-state-missing"
        source["consumption_request"]["parameter_context_id"] = "param-state-missing"
        _parameter_context_ref(source)["context_id"] = "param-state-missing"

        summary = build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)

        self.assertEqual(
            summary["classification"], "prepared_run_parameter_state_unavailable_for_review"
        )
        self.assertIn(
            "selected_parameter_state_missing_from_read_view",
            {finding["code"] for finding in summary["review_findings"]},
        )

    def test_trusted_entries_must_match_read_view_state(self) -> None:
        source = _load_input()
        source["source_agnostic_read_view_summary"]["stored_states"][1]["trusted_entries"].append(
            copy.deepcopy(
                source["source_agnostic_read_view_summary"]["stored_states"][1]["trusted_entries"][
                    0
                ]
            )
        )

        with self.assertRaisesRegex(ValueError, "duplicate trusted entry"):
            build_prepared_run_source_agnostic_parameter_state_consumption_summary(source)


if __name__ == "__main__":
    unittest.main()
